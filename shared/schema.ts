...
// Interface para FlowElementData (usada na tabela 'flows')
export interface FlowElementData {
  nodes: any[]; // Use tipos mais específicos se os tiver para nós do React Flow
  edges: any[]; // Use tipos mais específicos se os tiver para arestas do React Flow
}
...
export const creatives = pgTable("creatives", {
  ...
  campaignId: integer("campaign_id").references(() => campaigns.id, { onDelete: 'set null' }),
  name: text("name").notNull(),
  type: text("type", { enum: ["image", "video", "text", "carousel"] }).notNull(), // Adicionado ENUM para type
  fileUrl: text("file_url"),
  content: text("content"), // Para criativos de texto
  status: text("status", { enum: ["approved", "pending", "rejected"] }).default("pending").notNull(), // Adicionado ENUM para status
  platforms: jsonb("platforms").$type<string[]>().default([]).notNull(),
...
export const whatsappMessages = pgTable("whatsapp_messages", {
  ...
  direction: text("direction", { enum: ["incoming", "outgoing"] }).notNull(), // Adicionado ENUM para direction
...
export const alerts = pgTable("alerts", {
  ...
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  campaignId: integer("campaign_id").references(() => campaigns.id, { onDelete: 'set null' }),
  type: text("type", { enum: ["budget", "performance", "approval", "system"] }).notNull(), // Adicionado ENUM para type
  title: text("title").notNull(),
  message: text("message").notNull(),
  isRead: boolean("is_read").default(false).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
export const budgets = pgTable("budgets", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  campaignId: integer("campaign_id").references(() => campaigns.id, { onDelete: 'cascade' }), // cascade aqui é comum
  ...
  period: text("period", { enum: ["daily", "weekly", "monthly", "total"] }).notNull(), // Adicionado ENUM para period
...
export const landingPages = pgTable("landing_pages", {
  ...
  description: text("description"),
  grapesJsData: jsonb("grapes_js_data"), // Estrutura do GrapesJS
  status: text("status", { enum: ["draft", "published", "archived"] }).default("draft").notNull(), // Adicionado ENUM para status
...
export const chatMessages = pgTable('chat_messages', {
  ...
  attachmentUrl: text('attachment_url'), // Para futuros anexos
  timestamp: timestamp('timestamp', { withTimezone: true }).defaultNow().notNull(),
});
...
export const userRelations = relations(users, ({ many }) => ({
  ...
  flows: many(flows) // Adicionado flows
}));
...
export const flowRelations = relations(flows, ({ one }) => ({
  user: one(users, { fields: [flows.userId], references: [users.id] }),
  campaign: one(campaigns, { fields: [flows.campaignId], references: [campaigns.id] }),
}));
...
export const insertCreativeSchema = createInsertSchema(creatives, {
  ...
  type: z.enum(creatives.type.enumValues as [string, ...string[]]), // Correção para ENUM
  status: z.enum(creatives.status.enumValues as [string, ...string[]]).optional(), // Correção para ENUM
  platforms: z.preprocess(
    (val) => {
      if (Array.isArray(val)) { return val; }
...
export const insertLandingPageSchema = createInsertSchema(landingPages, {
  ...
  status: z.enum(landingPages.status.enumValues as [string, ...string[]]).optional(), // Correção para ENUM
...
export const insertWhatsappMessageSchema = createInsertSchema(whatsappMessages, {
  ...
  direction: z.enum(whatsappMessages.direction.enumValues as [string, ...string[]]), // Correção para ENUM
}).omit({ id: true, userId: true, timestamp: true, isRead: true });

export const insertAlertSchema = createInsertSchema(alerts, {
  type: z.enum(alerts.type.enumValues as [string, ...string[]]), // Correção para ENUM
...
export const insertBudgetSchema = createInsertSchema(budgets, {
  ...
  period: z.enum(budgets.period.enumValues as [string, ...string[]]), // Correção para ENUM
  startDate: z.preprocess(
    (arg) => { if (typeof arg === "string" || arg instanceof Date) return new Date(arg); return undefined; },
...
export const insertMetricSchema = createInsertSchema(metrics, {
  ...
  // userId: z.number().int().positive(), // Removido userId daqui, pois já está no InsertMetric original
  date: z.preprocess((arg) => { if (typeof arg === "string" || arg instanceof Date) return new Date(arg); return undefined; }, z.date()),
  impressions: z.number().int().min(0).default(0),
  clicks: z.number().int().min(0).default(0),
  conversions: z.number().int().min(0).default(0),
  cost: z.preprocess(
    (val) => (typeof val === 'string' && val.trim() !== '' ? parseFloat(val) : (typeof val === 'number' ? val : 0)),
...
export const insertFlowSchema = createInsertSchema(flows, {
  ...
  elements: z.custom<FlowElementData>((data) => {
    if (typeof data === 'object' && data !== null && 'nodes' in data && 'edges' in data) {
      return true;
    }
    return false;
  }, { message: "Elementos do fluxo devem conter 'nodes' e 'edges'."}).optional().default({ nodes: [], edges: [] }),
  campaignId: z.preprocess(
    (val) => (val === undefined || val === null || val === "" || String(val).toUpperCase() === "NONE" ? null : parseInt(String(val))),
    z.number().int().positive().nullable().optional()
  ),
...
export const aiResponseSchema = {
  ...
  properties: {
    ...
    alternativeVariation1: { type: "STRING" },
    alternativeVariation2: { type: "STRING" },
...
export const optimizeCopyResponseSchema = {
  type: "OBJECT" as const,
  properties: {
    optimizedCopy: { type: "STRING" as const },
    optimizationNotes: { type: "STRING" as const }
  },
  required: ["optimizedCopy"]
};