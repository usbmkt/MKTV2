...
// --- Definições de Tipos para Copy Configurations (Movido de client/src/config/copyConfigurations.ts) ---
...
// Schema para insertBudgetSchema (Necessário para server/routes.ts)
...
// Schema para insertChatSessionSchema (Necessário para server/routes.ts)
...
export type InsertBudget = z.infer<typeof insertBudgetSchema>; // Exportando o tipo inferido
...
export type InsertChatSession = z.infer<typeof insertChatSessionSchema>; // Exportando o tipo inferido
...
// --- CONFIGURAÇÕES DE COPY (Movido de client/src/config/copyConfigurations.ts) ---
...
// Este é o schema que o backend espera que a IA retorne.
...
// Se precisar de um schema Zod para validar a resposta da IA internamente no backend (opcional):
...
// Outros schemas que podem ser úteis (exemplo)
