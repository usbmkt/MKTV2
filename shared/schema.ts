...
export const users = pgTable("users", {
  ...
  password: text("password").notNull(), // Lembre-se de fazer hash no backend
...
export const copies = pgTable("copies", {
  ...
  purposeKey: text("purpose_key").notNull(), // Ex: 'headline', 'email_subject', 'ad_copy'
  launchPhase: launchPhaseEnum("launch_phase").notNull(), // 'pre_launch', 'launch', 'post_launch'
  details: jsonb("details").$type<Record<string, any>>().default({}), // Campos específicos do 'purposeKey'
  baseInfo: jsonb("base_info").$type<any>().default({}), // Informações base usadas para gerar a copy
  fullGeneratedResponse: jsonb("full_generated_response").$type<any>().default({}), // Resposta completa da IA
  platform: text("platform"), // Ex: 'facebook', 'google_ads', 'email'
...
export const creatives = pgTable("creatives", {
  ...
  fileUrl: text("file_url"), // Para image/video
  content: text("content"), // Para text
...
export const metrics = pgTable("metrics", {
  ...
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }), // Denormalized for easier user-specific queries if needed
  ...
  // Add other relevant metrics: cpc, ctr, cpa, roas (can be calculated or stored)
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const whatsappMessages = pgTable("whatsapp_messages", {
  ...
  // campaignId: integer("campaign_id").references(() => campaigns.id), // Optional: link to campaign
});
...
export const budgets = pgTable("budgets", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  campaignId: integer("campaign_id").references(() => campaigns.id, { onDelete: 'cascade' }), // Budget is often tied to a campaign
  ...
  period: text("period", { enum: ["daily", "weekly", "monthly", "total"] }).notNull(), // 'total' for lifetime budget
  startDate: timestamp("start_date", { withTimezone: true }).notNull(),
  endDate: timestamp("end_date", { withTimezone: true }), // Nullable for ongoing budgets
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const landingPages = pgTable("landing_pages", {
    ...
    userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
    name: text("name").notNull(),
    studioProjectId: varchar("studio_project_id", { length: 255 }).unique(), // ID from GrapesJS Studio or similar
    slug: varchar("slug", { length: 255 }).notNull().unique(), // For the public URL
    description: text("description"),
    grapesJsData: jsonb("grapes_js_data"), // Stores the GrapesJS project data (JSON)
    // htmlContent: text("html_content"), // Generated HTML (optional, can be built on demand)
    // cssContent: text("css_content"), // Generated CSS (optional)
    status: text("status", { enum: ["draft", "published", "archived"] }).default("draft").notNull(),
    publicUrl: text("public_url"), // Full public URL after publishing
...
export const chatMessages = pgTable('chat_messages', {
  ...
  sender: chatSenderEnum('sender').notNull(), // 'user' or 'agent'
  text: text('text').notNull(),
  attachmentUrl: text('attachment_url'), // For images or files sent in chat
  timestamp: timestamp('timestamp', { withTimezone: true }).defaultNow().notNull(),
});
...
export const funnels = pgTable("funnels", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  campaignId: integer("campaign_id").references(() => campaigns.id, { onDelete: 'set null' }), // Funnel can be optionally linked to a campaign
  name: text("name").notNull(),
  description: text("description"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
...
export const funnelStages = pgTable("funnel_stages", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  order: integer("order").notNull().default(0), // To define the sequence of stages
  config: jsonb("config").$type<Record<string, any>>(), // For specific stage configurations, e.g., conversion goals
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const campaignRelations = relations(campaigns, ({ one, many }) => ({
  ...
  alerts: many(alerts),
  budgets: many(budgets),
  funnels: many(funnels), // Campaign can have multiple funnels
}));
...
export const insertCampaignSchema = createInsertSchema(campaigns, {
  ...
  id: true, createdAt: true, updatedAt: true, userId: true // userId será adicionado no backend
});
...
export const insertCopySchema = createInsertSchema(copies, {
  ...
  userId: true, // userId será adicionado no backend
});
...
export const insertFunnelSchema = createInsertSchema(funnels, {
  ...
  campaignId: z.preprocess(
    (val) => {
      ...
      const parsed = parseInt(String(val)); // String(val) para garantir que é string antes do parseInt
...
export const insertFunnelStageSchema = createInsertSchema(funnelStages, {
    ...
    // funnelId é obrigatório e deve ser fornecido ao criar/atualizar
...
export const insertLandingPageSchema = createInsertSchema(landingPages, {
  ...
  studioProjectId: z.string().optional().nullable(), // Pode ser opcional inicialmente
}).omit({
  id: true, createdAt: true, updatedAt: true, userId: true, publicUrl: true, publishedAt: true
});
...
// export const insertMetricSchema = createInsertSchema(metrics); // Defina se precisar de validação específica para inserção
...
export type Funnel = z.infer<typeof selectFunnelSchema>; // Usado como FunnelType em funnel.tsx
export type InsertFunnel = z.infer<typeof insertFunnelSchema>; // Necessário para o formulário em funnel.tsx

export type FunnelStage = z.infer<typeof selectFunnelStageSchema>; // Usado como FunnelStage em funnel.tsx
