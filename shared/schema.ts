...
export const userRelations = relations(users, ({ many }) => ({
  ...
  whatsappMessages: many(whatsappMessages), // Referência para a tabela abaixo
  copies: many(copies),
  alerts: many(alerts),
...
export const whatsappMessages = pgTable("whatsapp_messages", { // Definição da tabela
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
...
export const whatsappMessageRelations = relations(whatsappMessages, ({ one }) => ({ // Relações
...
export const insertCampaignSchema = createInsertSchema(campaigns, {
  ...
  budget: z.preprocess((val) => (val === "" || val === null || val === undefined ? null : String(val)), z.string().nullable().optional()),
  dailyBudget: z.preprocess((val) => (val === "" || val === null || val === undefined ? null : String(val)), z.string().nullable().optional()),
  avgTicket: z.preprocess((val) => (val === "" || val === null || val === undefined ? null : String(val)), z.string().nullable().optional()),
  startDate: z.preprocess((arg) => (typeof arg === "string" || arg instanceof Date ? new Date(arg) : undefined), z.date().optional().nullable()),
  endDate: z.preprocess((arg) => (typeof arg === "string" || arg instanceof Date ? new Date(arg) : undefined), z.date().optional().nullable()),
}).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertCreativeSchema = createInsertSchema(creatives, {
  ...
  platforms: z.preprocess((val) => (Array.isArray(val) ? val : typeof val === 'string' ? val.split(',').map(s => s.trim()).filter(s => s.length > 0) : []), z.array(z.string()).optional()),
  fileUrl: z.string().nullable().optional(),
  campaignId: z.preprocess((val) => (val === "null" || val === "" ? null : typeof val === 'number' || val === null ? val : typeof val === 'string' ? (isNaN(parseInt(val)) ? undefined : parseInt(val)) : undefined), z.number().nullable().optional()),
}).omit({ id: true, createdAt: true, updatedAt: true });

export const insertMetricSchema = createInsertSchema(metrics).omit({ id: true, createdAt: true });

export const insertWhatsappMessageSchema = createInsertSchema(whatsappMessages, { // Schema Zod
...
export const insertBudgetSchema = createInsertSchema(budgets, {
  totalBudget: z.preprocess((val) => String(val), z.string({invalid_type_error: "Orçamento total deve ser um texto"})),
  spentAmount: z.preprocess((val) => String(val), z.string({invalid_type_error: "Valor gasto deve ser um texto"}).optional()),
  startDate: z.preprocess((arg) => (typeof arg === "string" || arg instanceof Date ? new Date(arg) : undefined), z.date()),
  endDate: z.preprocess((arg) => (typeof arg === "string" || arg instanceof Date ? new Date(arg) : undefined), z.date().optional().nullable()),
}).omit({ id: true, createdAt: true });
...
export type WhatsappMessage = typeof whatsappMessages.$inferSelect; // Tipo Drizzle
export type InsertWhatsappMessage = z.infer<typeof insertWhatsappMessageSchema>; // Tipo Zod
...
// Adicionando um export default de todos os schemas para facilitar a importação com '*'
const allSchemas = {
  campaignStatusEnum,
  chatSenderEnum,
  users,
  ...
  metrics,
  metricRelations,
  whatsappMessages, // Incluído aqui
  whatsappMessageRelations,
  copies,
  copyRelations,
  alerts,
  alertRelations,
  ...
  insertMetricSchema,
  insertWhatsappMessageSchema, // Incluído aqui
  insertCopySchema,
  insertAlertSchema,
  insertBudgetSchema,
...
export default allSchemas; // Exportação default
