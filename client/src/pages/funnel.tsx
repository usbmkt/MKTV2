...
export default function FunnelPage() {
  ...
  const funnelChartData = useMemo(() => {
    ...
    let currentStageValue = selectedFunnelData.totalVisitors || 1000; // Base para simulação
    return selectedFunnelData.stages
      .sort((a, b) => a.order - b.order)
      .map((stage, index) => {
        if (index > 0) currentStageValue = Math.max(1, Math.floor(currentStageValue * (0.4 + Math.random() * 0.55))); // Queda simulada
    ...
    <div className="space-y-6 p-4 md:p-8">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          {isLoadingSelectedFunnel && selectedFunnelId && <div className="p-8 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto text-primary" /> Carregando dados do funil...</div>}
          ...
          {selectedFunnelData && !isLoadingSelectedFunnel && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                 {/* Cards de KPI atualizados para usar dados do funil selecionado ou do gráfico simulado */}
              ...
              <Card>
                ...
                <CardContent className="h-[400px] md:h-[500px] p-2">
                  ...
                  ) : <div className="flex items-center justify-center h-full text-muted-foreground">Este funil não possui etapas ou dados para visualização.</div>}
                </CardContent>
              </Card>
            </>
          )}
        ...
        <TabsContent value="detailed" className="space-y-4">
          {/* ... (Conteúdo da Aba Análise Detalhada - pode ser aprimorado depois para usar dados reais de stage_conversions se disponíveis) ... */}
        </TabsContent>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmitFunnelForm)} className="space-y-4 py-4">
              ...
              <FormField control={form.control} name="description" render={({ field }) => (<FormItem><FormLabel>Descrição</FormLabel><FormControl><Textarea placeholder="Descreva o funil..." {...field} value={field.value ?? ''}/></FormControl><FormMessage /></FormItem>)} />
