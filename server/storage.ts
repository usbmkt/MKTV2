...
export class DatabaseStorage implements IStorage {
  ...
  async getCopies(userId: number, campaignId?: number | null, phase?: string, purposeKey?: string, searchTerm?: string): Promise<schema.Copy[]> { const conditions: any[] = [eq(schema.copies.userId, userId)]; if (campaignId !== undefined) { conditions.push(campaignId === null ? isNull(schema.copies.campaignId) : eq(schema.copies.campaignId, campaignId)); } if (phase && phase !== 'all') { conditions.push(eq(schema.copies.launchPhase, phase as typeof schema.launchPhaseEnum.enumValues[number]));} if (purposeKey && purposeKey !== 'all') { conditions.push(eq(schema.copies.purposeKey, purposeKey)); } if (searchTerm && searchTerm.trim() !== '') { const searchPattern = `%${searchTerm.toLowerCase()}%`; conditions.push( or( ilike(schema.copies.title, searchPattern), ilike(schema.copies.content, searchPattern) )); } return db.select().from(schema.copies).where(and(...conditions)).orderBy(desc(schema.copies.createdAt)); }
  async createCopy(copyData: schema.InsertCopy): Promise<schema.Copy> { const dataToInsert = { ...copyData, campaignId: copyData.campaignId === undefined ? null : copyData.campaignId, details: copyData.details || {}, baseInfo: copyData.baseInfo || {}, fullGeneratedResponse: copyData.fullGeneratedResponse || {}, tags: copyData.tags || [], }; const [newCopy] = await db.insert(schema.copies).values(dataToInsert).returning(); if (!newCopy) { throw new Error("Falha ao salvar a copy no banco de dados.");} return newCopy; }
  async updateCopy(id: number, copyData: Partial<Omit<schema.InsertCopy, 'userId' | 'id' | 'createdAt'>>, userId: number): Promise<schema.Copy | undefined> { const existingCopyResult = await db.select({id: schema.copies.id}).from(schema.copies).where(and(eq(schema.copies.id, id), eq(schema.copies.userId, userId))).limit(1); if(!existingCopyResult || existingCopyResult.length === 0) { return undefined; } const dataToUpdate: Partial<Omit<schema.Copy, 'id' | 'userId' | 'createdAt'>> = { ...copyData, lastUpdatedAt: new Date(), }; if (copyData.hasOwnProperty('campaignId')) { (dataToUpdate as any).campaignId = copyData.campaignId === undefined ? null : copyData.campaignId; } const [updatedCopy] = await db.update(schema.copies).set(dataToUpdate).where(and(eq(schema.copies.id, id), eq(schema.copies.userId, userId))).returning(); return updatedCopy; }
  ...
  async getDashboardData(userId: number, timeRange: string = '30d'): Promise<any> { const now = new Date(); let startDate = new Date(); if (timeRange === '7d') { startDate.setDate(now.getDate() - 7); } else { startDate.setDate(now.getDate() - 30); } const metricsTimeCondition = and( eq(schema.metrics.userId, userId), gte(schema.metrics.date, startDate) ); const budgetsUserCondition = eq(schema.budgets.userId, userId); const activeCampaignsResult = await db.select({ count: count() }).from(schema.campaigns).where(and(eq(schema.campaigns.userId, userId), eq(schema.campaigns.status, 'active'))); const activeCampaigns = activeCampaignsResult[0]?.count || 0; const totalSpentResult = await db.select({ total: sum(sql<string | number>`CAST(${schema.budgets.spentAmount} AS DECIMAL)`) }).from(schema.budgets).where(budgetsUserCondition); const totalSpent = parseFloat(String(totalSpentResult[0]?.total || "0")) || 0; const totalConversionsResult = await db.select({ total: sum(schema.metrics.conversions) }).from(schema.metrics).where(metricsTimeCondition); const conversions = parseFloat(String(totalConversionsResult[0]?.total || '0')) || 0; const totalRevenueResult = await db.select({ total: sum(schema.metrics.revenue) }).from(schema.metrics).where(metricsTimeCondition); const totalRevenue = parseFloat(String(totalRevenueResult[0]?.total || '0')) || 0; const totalCostResult = await db.select({ total: sum(schema.metrics.cost) }).from(schema.metrics).where(metricsTimeCondition); const totalCost = parseFloat(String(totalCostResult[0]?.total || '0')) || 0; const avgROI = totalCost > 0 ? parseFloat((((totalRevenue - totalCost) / totalCost) * 100).toFixed(2)) : 0; const totalImpressionsResult = await db.select({ total: sum(schema.metrics.impressions) }).from(schema.metrics).where(metricsTimeCondition); const impressions = parseFloat(String(totalImpressionsResult[0]?.total || '0')) || 0; const totalClicksResult = await db.select({ total: sum(schema.metrics.clicks) }).from(schema.metrics).where(metricsTimeCondition); const clicks = parseFloat(String(totalClicksResult[0]?.total || '0')) || 0; const ctr = clicks > 0 && impressions > 0 ? parseFloat(((clicks / impressions) * 100).toFixed(2)) : 0; const cpc = clicks > 0 && totalCost > 0 ? parseFloat((totalCost / clicks).toFixed(2)) : 0; const metricsData = { activeCampaigns, totalSpent, totalCostPeriod: totalCost, conversions, avgROI, impressions, clicks, ctr, cpc }; const campaignsChange = parseFloat((Math.random() * 20 - 10).toFixed(1)); const spentChange = parseFloat((Math.random() * 20 - 10).toFixed(1)); const conversionsChange = parseFloat((Math.random() * 30 - 15).toFixed(1)); const roiChange = parseFloat((Math.random() * 10 - 5).toFixed(1)); const trends = { campaignsChange, spentChange, conversionsChange, roiChange, }; const recentCampaignsRaw = await db.select().from(schema.campaigns).where(eq(schema.campaigns.userId, userId)).orderBy(desc(schema.campaigns.createdAt)).limit(3); const recentCampaigns = recentCampaignsRaw.map(c => ({ id: c.id, name: c.name, description: c.description || 'Nenhuma descrição', status: c.status, platforms: c.platforms || [], budget: parseFloat(String(c.budget ?? '0')) || 0, spent: parseFloat(String(c.dailyBudget ?? '0')) || 0, performance: Math.floor(Math.random() * (95 - 60 + 1)) + 60 })); const timeSeriesData = generateSimulatedLineChartData('Desempenho Geral', 1000, timeRange === '30d' ? 30 : 7, 50, chartColors.palette[0]); const channelPerformanceData = generateSimulatedDoughnutChartData(['Meta Ads', 'Google Ads', 'LinkedIn', 'TikTok'], 20, 10, chartColors.palette); const conversionData = generateSimulatedLineChartData('Conversões', 200, timeRange === '30d' ? 30 : 7, 30, chartColors.palette[1]); const roiData = generateSimulatedBarChartData('ROI (%)', ['Meta Ads', 'Google Ads', 'LinkedIn', 'TikTok'], 250, 100, chartColors.palette); return { metrics: metricsData, recentCampaigns, alertCount: (await db.select({ count: count() }).from(schema.alerts).where(and(eq(schema.alerts.userId, userId), eq(schema.alerts.isRead, false))))[0]?.count || 0, trends, timeSeriesData, channelPerformanceData, conversionData, roiData, }; }
