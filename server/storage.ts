...
import { eq, and, sql, desc, isNull, or, count } from 'drizzle-orm';
...
export const storage = {
    ...
    async getCopies(userId: number, campaignId?: number, phase?: string, purpose?: string, search?: string): Promise<Copy[]> {
        ...
        if (search) conditions.push(or(sql.raw(`"title" ILIKE ?`, `%${search}%`), sql.raw(`"content" ILIKE ?`, `%${search}%`)));
        return await db.query.copies.findMany({ where: and(...conditions), orderBy: [desc(schema.copies.createdAt)] });
    },
    ...
    async findTriggerFlow(userId: number, triggerKeyword: string): Promise<Flow | undefined> {
        // Implementar a lógica para encontrar um fluxo pelo gatilho.
        // Por agora, um mock simples.
