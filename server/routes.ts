...
export function registerRoutes(app: Express): HttpServer {
    ...
    apiRouter.get('/campaigns/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da campanha inválido.' }); const campaign = await storage.getCampaign(id, req.user.id); res.json(campaign); } catch(e) { next(e); } });
    apiRouter.put('/campaigns/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da campanha inválido.' }); const data = schemaShared.insertCampaignSchema.partial().parse(req.body); const campaign = await storage.updateCampaign(id, data, req.user.id); res.json(campaign); } catch (e) { next(e); } });
    apiRouter.delete('/campaigns/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da campanha inválido.' }); await storage.deleteCampaign(id, req.user.id); res.status(204).send(); } catch (e) { next(e); } });
    apiRouter.get('/creatives', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const campaignIdQuery = req.query.campaignId as string; const campaignId = campaignIdQuery === 'null' ? null : (campaignIdQuery ? parseInt(campaignIdQuery) : undefined); const creatives = await storage.getCreatives(req.user.id, campaignId); res.json(creatives); } catch (e) { next(e); } });
    ...
    apiRouter.put('/creatives/:id', creativesUpload.single('file'), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID do criativo inválido.' }); const updateData = schemaShared.insertCreativeSchema.partial().parse({ ...req.body, fileUrl: req.file ? `/${UPLOADS_ROOT_DIR}/creatives-assets/${req.file.filename}` : req.body.fileUrl }); const updated = await storage.updateCreative(id, updateData, req.user.id); if (!updated) return res.status(404).json({error: "Criativo não encontrado"}); res.json(updated); } catch(e) { next(e); } });
    apiRouter.delete('/creatives/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID do criativo inválido.' }); await storage.deleteCreative(id, req.user.id); res.status(204).send(); } catch (e) { next(e); } });
    apiRouter.get('/funnels', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getFunnels(req.user.id)); } catch (e) { next(e); } });
    apiRouter.post('/funnels', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const data = schemaShared.insertFunnelSchema.parse(req.body); const funnel = await storage.createFunnel({ ...data, userId: req.user.id }); res.status(201).json(funnel); } catch(e) { next(e); } });
    apiRouter.get('/copies', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const { campaignId, phase, purpose, search } = req.query; const copies = await storage.getCopies( req.user.id, campaignId ? Number(campaignId) : undefined, phase as string, purpose as string, search as string ); res.json(copies); } catch (e) { next(e); } });
    apiRouter.post('/copies', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const data = schemaShared.insertCopySchema.parse(req.body); const copy = await storage.createCopy({ ...data, userId: req.user.id }); res.status(201).json(copy); } catch (e) { next(e); } });
    apiRouter.delete('/copies/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da cópia inválido.' }); await storage.deleteCopy(id, req.user.id); res.status(204).send(); } catch (e) { next(e); } });
    apiRouter.get('/landingpages', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getLandingPages(req.user.id)); } catch (e) { next(e); } });
    apiRouter.post('/landingpages', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const lpData = schemaShared.insertLandingPageSchema.parse({ ...req.body }); const landingPage = await storage.createLandingPage({ ...lpData, userId: req.user.id }); res.status(201).json(landingPage); } catch(e) { next(e); } });
    apiRouter.get('/landingpages/studio-project/:studioProjectId', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const lp = await storage.getLandingPageByStudioProjectId(req.params.studioProjectId, req.user.id); if (!lp) return res.status(404).json({ error: 'Projeto não encontrado.' }); res.json({ project: lp.grapesJsData || {} }); } catch(e) { next(e); } });
    apiRouter.put('/landingpages/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da landing page inválido.' }); const lpData = schemaShared.insertLandingPageSchema.partial().parse(req.body); const landingPage = await storage.updateLandingPage(id, lpData, req.user.id); res.json(landingPage); } catch(e) { next(e); } });
    apiRouter.delete('/landingpages/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID da landing page inválido.' }); await storage.deleteLandingPage(id, req.user.id); res.status(204).send(); } catch(e) { next(e); } });
    apiRouter.post('/assets/lp-upload', lpAssetUpload.single('file'), (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.file) return res.status(400).json({ error: "Nenhum arquivo enviado." }); const appBaseUrl = process.env.APP_BASE_URL || `http://localhost:${process.env.PORT || 5000}`; const publicUrl = `${appBaseUrl}/${UPLOADS_ROOT_DIR}/lp-assets/${req.file.filename}`; res.status(200).json([{ src: publicUrl }]); } catch(e) { next(e); } });
    apiRouter.post('/assets/lp-delete', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { const { assets } = req.body; if (Array.isArray(assets)) { assets.forEach(asset => { try { const filename = path.basename(new URL(asset.src).pathname); const filePath = path.join(LP_ASSETS_DIR, filename); if (fs.existsSync(filePath)) fs.unlinkSync(filePath); } catch (e) { logger.warn({ asset: asset.src, error: e}, `Erro ao deletar asset`); } }); } res.status(200).json({ message: "Solicitação processada." }); } catch(e) { next(e); } });
    apiRouter.get('/budgets', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getBudgets(req.user.id)); } catch (e) { next(e); } });
    apiRouter.post('/budgets', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const data = schemaShared.insertBudgetSchema.parse(req.body); const budget = await storage.createBudget({ ...data, userId: req.user.id }); res.status(201).json(budget); } catch (e) { next(e); } });
    apiRouter.get('/alerts', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getAlerts(req.user.id, req.query.unread === 'true')); } catch(e) { next(e); } });
    apiRouter.put('/alerts/:id/read', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID do alerta inválido.' }); await storage.markAlertAsRead(id, req.user.id); res.status(200).json({ success: true }); } catch (e) { next(e); } });
    ...
    apiRouter.put('/flows/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID do fluxo inválido.' }); const dataToParse = { ...req.body, elements: req.body.elements || undefined }; const data = schemaShared.insertFlowSchema.partial().parse(dataToParse); const flow = await storage.updateFlow(id, data, req.user.id); res.json(flow); } catch(e) { next(e); } });
    apiRouter.delete('/flows/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: 'ID do fluxo inválido.' }); await storage.deleteFlow(id, req.user.id); res.status(204).send(); } catch(e) { next(e); } });
    apiRouter.post('/whatsapp/connect', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const service = getWhatsappServiceForUser(req.user.id); await service.connectToWhatsApp(); res.status(202).json({ message: "Iniciando conexão." }); } catch(e) { next(e); } });
    apiRouter.get('/whatsapp/status', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(WhatsappConnectionService.getStatus(req.user.id)); } catch(e) { next(e); } });
    apiRouter.post('/whatsapp/disconnect', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const service = getWhatsappServiceForUser(req.user.id); await service.disconnectWhatsApp(); res.json({ message: "Desconexão solicitada." }); } catch(e) { next(e); } });
    apiRouter.get('/whatsapp/contacts', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getContacts(req.user.id)); } catch(e) { next(e); } });
    apiRouter.get('/whatsapp/messages', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const contactNumber = req.query.contactNumber as string; if(!contactNumber) return res.status(400).json({ error: "Número do contato é obrigatório."}); res.json(await storage.getMessages(req.user.id, contactNumber)); } catch(e) { next(e); } });
    apiRouter.post('/whatsapp/messages', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const { contactNumber, message } = schemaShared.insertWhatsappMessageSchema.pick({ contactNumber: true, message: true }).parse(req.body); const service = getWhatsappServiceForUser(req.user.id); const fullJid = contactNumber.endsWith('@s.whatsapp.net') ? contactNumber : `${contactNumber}@s.whatsapp.net`; await service.sendMessage(fullJid, { text: message }); const savedMessage = await storage.createMessage({ userId: req.user.id, contactNumber, message, direction: 'outgoing' as const }); if (savedMessage) { io.to(`user_${req.user.id}`).emit('new_message', savedMessage); res.status(201).json(savedMessage); } else { res.status(500).json({ error: 'Erro ao salvar mensagem.' }); } } catch (e) { next(e); } });
    apiRouter.get('/whatsapp/templates', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getWhatsappTemplates(req.user.id)); } catch (e) { next(e); } });
    apiRouter.post('/whatsapp/templates', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const data = schemaShared.insertWhatsappMessageTemplateSchema.parse(req.body); const newTemplate = await storage.createWhatsappTemplate({ ...data, userId: req.user.id }); res.status(201).json(newTemplate); } catch (e) { next(e); } });
    apiRouter.put('/whatsapp/templates/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: "ID do template inválido." }); const data = schemaShared.insertWhatsappMessageTemplateSchema.partial().parse(req.body); const updatedTemplate = await storage.updateWhatsappTemplate(id, data, req.user.id); if (!updatedTemplate) return res.status(404).json({ error: "Template não encontrado." }); res.json(updatedTemplate); } catch (e) { next(e); } });
    apiRouter.delete('/whatsapp/templates/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const id = parseInt(req.params.id); if (isNaN(id)) return res.status(400).json({ error: "ID do template inválido." }); const success = await storage.deleteWhatsappTemplate(id, req.user.id); if (!success) return res.status(404).json({ error: "Template não encontrado." }); res.status(204).send(); } catch (e) { next(e); } });
    ...
    apiRouter.get('/chat/sessions', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); res.json(await storage.getChatSessions(req.user.id)); } catch (e) { next(e); } });
    apiRouter.post('/chat/sessions', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const data = schemaShared.insertChatSessionSchema.parse(req.body); const session = await storage.createChatSession(req.user.id, data.title); res.status(201).json(session); } catch (e) { next(e); } });
    apiRouter.get('/chat/sessions/:sessionId/messages', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const sessionId = parseInt(req.params.sessionId); if (isNaN(sessionId)) return res.status(400).json({ error: 'ID da sessão inválido.' }); res.json(await storage.getChatMessages(sessionId, req.user.id)); } catch (e) { next(e); } });
    apiRouter.put('/chat/sessions/:sessionId/title', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const sessionId = parseInt(req.params.sessionId); if (isNaN(sessionId)) return res.status(400).json({ error: 'ID da sessão inválido.' }); const { title } = req.body; if (!title || typeof title !== 'string') return res.status(400).json({ error: 'Título é obrigatório.' }); const updated = await storage.updateChatSessionTitle(sessionId, req.user.id, title); if (!updated) return res.status(404).json({ error: 'Sessão não encontrada.' }); res.json(updated); } catch (e) { next(e); } });
    apiRouter.delete('/chat/sessions/:sessionId', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { if (!req.user) return res.status(401).json({ error: 'Usuário não autenticado.' }); const sessionId = parseInt(req.params.sessionId); if (isNaN(sessionId)) return res.status(400).json({ error: 'ID da sessão inválido.' }); await storage.deleteChatSession(sessionId, req.user.id); res.status(204).send(); } catch (e) { next(e); } });
