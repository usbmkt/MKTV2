...
export function registerRoutes(app: Express): HttpServer {
    ...
    apiRouter.post('/landingpages', async (req: AuthenticatedRequest, res, next) => { try { const lpData = schemaShared.insertLandingPageSchema.parse({ ...req.body }); res.status(201).json(await storage.createLandingPage({ ...lpData, userId: req.user!.id })); } catch(e) { next(e); }});
    ...
    apiRouter.put('/flows/:id', async (req: AuthenticatedRequest, res, next) => { try { const data = schemaShared.insertFlowSchema.partial().parse(req.body); res.json(await storage.updateFlow(parseInt(req.params.id), data, req.user!.id)); } catch(e) { next(e); }});
