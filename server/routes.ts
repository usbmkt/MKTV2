...
export async function registerRoutes(app: Express): Promise<HttpServer> {
  ...
  let localGenAIForCopies: GoogleGenerativeAI | null = null; if (GEMINI_API_KEY && GEMINI_API_KEY !== "SUA_CHAVE_API_GEMINI_AQUI" && GEMINI_API_KEY.length > 10) { try { localGenAIForCopies = new GoogleGenerativeAI(GEMINI_API_KEY); } catch (error) { console.error("[COPIES_ROUTE_GEMINI] Falha ao init SDK:", error); } }
  app.post('/api/copies/generate', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { const { product, audience, objective, tone } = req.body; if (!product || !audience || !objective || !tone) return res.status(400).json({ error: "Campos obrigatórios." }); if (!localGenAIForCopies) return res.status(500).json({ error: "IA não disponível." }); const model = localGenAIForCopies.getGenerativeModel({ model: "gemini-1.5-flash-latest" }); const prompts = [ { type: 'headline', platform: 'Facebook', prompt: `Crie um headline para Facebook Ads: Produto="${product}", Público="${audience}", Objetivo="${objective}", Tom="${tone}". Máx 60 chars.` }, { type: 'cta', platform: 'Google Ads', prompt: `Crie um CTA para Google Ads: Produto="${product}", Público="${audience}", Objetivo="${objective}", Tom="${tone}". Máx 30 palavras.` }, { type: 'description', platform: 'Instagram', prompt: `Crie uma descrição para Instagram: Produto="${product}", Público="${audience}", Objetivo="${objective}", Tom="${tone}". Máx 125 chars.` } ]; const results = await Promise.all(prompts.map(async p => { try { return { type: p.type, content: (await model.generateContent(p.prompt)).response.text().trim(), platform: p.platform }; } catch (e) { return { type: p.type, content: `Fallback: ${product} para ${audience} (${p.type})`, platform: p.platform }; } })); res.json(results); } catch (error) { next(error); }});
  app.post('/api/mcp/converse', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => { try { const { message, sessionId, attachmentUrl } = req.body; const userId = req.user!.id; const result = await handleMCPConversation(userId, message, sessionId, attachmentUrl); res.json(result); } catch (error) { next(error); }});
  ...
  app.get('/api/funnels', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const campaignIdQuery = req.query.campaignId as string | undefined;
      const campaignId = campaignIdQuery === 'null' || campaignIdQuery === '' ? null : (campaignIdQuery ? parseInt(campaignIdQuery) : undefined);
      if (campaignIdQuery && campaignIdQuery !== 'null' && campaignIdQuery !== '' && isNaN(campaignId!)) {
        return res.status(400).json({ error: 'ID da campanha inválido para filtro de funis.' });
      }
  ...
  app.put('/api/funnels/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const updatedFunnel = await storage.updateFunnel(id, funnelData, req.user!.id);
  ...
  app.get('/api/funnels/:funnelId/stages', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // A verificação de propriedade do funil já ocorre em storage.getFunnelStages
        const stages = await storage.getFunnelStages(funnelId, req.user!.id);
        res.json(stages);
    } catch (error) {
        next(error);
    }
  ...
  app.post('/api/funnels/:funnelId/stages', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const funnel = await storage.getFunnel(funnelId, req.user!.id); // Verifica se o funil existe e pertence ao usuário
      if (!funnel) return res.status(404).json({ error: 'Funil não encontrado ou não pertence ao usuário.' });
      
      const stageData = insertFunnelStageSchema.parse({ ...req.body, funnelId }); // funnelId é adicionado aqui
      const newStage = await storage.createFunnelStage(stageData);
  ...
  app.put('/api/stages/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const updatedStage = await storage.updateFunnelStage(id, stageData, req.user!.id); // Passa userId para verificação de permissão
  ...
  app.delete('/api/stages/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) return res.status(400).json({ error: 'ID da etapa inválido.' });
      const success = await storage.deleteFunnelStage(id, req.user!.id); // Passa userId para verificação de permissão
