...
if (GEMINI_API_KEY) { 
    try { 
        ...
        console.error("Falha ao inicializar GoogleGenerativeAI:", error); 
    } 
}
...
async function doRegisterRoutes(app: Express): Promise<HttpServer> {
    ...
    apiRouter.post('/whatsapp/messages', async (req: AuthenticatedRequest, res, next) => {
        try {
            ...
            // Adiciona o sufixo @s.whatsapp.net se não estiver presente
            ...
            const savedMessage = await storage.createMessage({ contactNumber, message, direction: 'outgoing', userId: req.user!.id });
