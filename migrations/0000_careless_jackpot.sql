-- Criação de ENUMs de forma idempotente
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'campaign_status' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'chat_sender' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'flow_status' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'launch_phase' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
-- Criação de Tabelas de forma idempotente
CREATE TABLE IF NOT EXISTS "public"."alerts" (
	"id" serial PRIMARY KEY NOT NULL,
	"user_id" integer NOT NULL,
	"campaign_id" integer,
	"type" text NOT NULL, -- Considere usar pgEnum aqui se os valores forem fixos
	"title" text NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."budgets" (
	...
	"period" text NOT NULL, -- Considere usar pgEnum aqui se os valores forem fixos
...
CREATE TABLE IF NOT EXISTS "public"."creatives" (
	...
	"campaign_id" integer,
	"name" text NOT NULL,
	"type" text NOT NULL, -- Se "type" tiver valores fixos (image, video, etc.), considere pgEnum
	"file_url" text,
	"content" text,
	"status" text DEFAULT 'pending' NOT NULL, -- Se "status" tiver valores fixos (approved, pending, etc.), considere pgEnum
	"platforms" jsonb DEFAULT '[]'::jsonb NOT NULL,
	"thumbnail_url" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
...
CREATE TABLE IF NOT EXISTS "public"."flows" (
	...
	"campaign_id" integer,
	"name" text NOT NULL,
	"status" public."flow_status" DEFAULT 'draft' NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."landing_pages" (
	...
	"status" text DEFAULT 'draft' NOT NULL, -- Se "status" tiver valores fixos (draft, published, etc.), considere pgEnum
	"public_url" text,
...
CREATE TABLE IF NOT EXISTS "public"."whatsapp_messages" (
	...
	"contact_name" text,
	"message" text NOT NULL,
	"direction" text NOT NULL, -- Se "direction" tiver valores fixos (incoming, outgoing), considere pgEnum
	"timestamp" timestamp with time zone DEFAULT now() NOT NULL,
	"is_read" boolean DEFAULT false NOT NULL
);
--> statement-breakpoint

-- Bloco para garantir que a coluna 'status' exista em 'campaigns'
DO $$
BEGIN
    -- Garante que o tipo ENUM campaign_status exista
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'campaign_status' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        CREATE TYPE "public"."campaign_status" AS ENUM('active', 'paused', 'completed', 'draft');
    END IF;

    -- Adiciona a coluna 'status' à tabela 'campaigns' somente se ela não existir
    -- Isso é feito após o CREATE TABLE IF NOT EXISTS para cobrir o caso da tabela existir mas sem a coluna.
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'campaigns' AND table_schema = 'public') THEN
        IF NOT EXISTS (
            ...
            ALTER TABLE "public"."campaigns" ADD COLUMN "status" public."campaign_status" DEFAULT 'draft' NOT NULL;
        END IF;
    END IF;
END
$$;
...
-- Bloco para garantir que a coluna 'sender' exista em 'chat_messages'
DO $$
BEGIN
    -- Garante que o tipo ENUM chat_sender exista
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'chat_sender' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        CREATE TYPE "public"."chat_sender" AS ENUM('user', 'agent');
    END IF;

    -- Adiciona a coluna 'sender' à tabela 'chat_messages' somente se ela não existir
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'chat_messages' AND table_schema = 'public') THEN
        IF NOT EXISTS (
            ...
            ALTER TABLE "public"."chat_messages" ADD COLUMN "sender" public."chat_sender"; -- Removido NOT NULL temporariamente se causar problemas em tabelas existentes
        END IF;
        -- Garante que a coluna seja NOT NULL se ela existir
        -- Cuidado: Se a coluna foi adicionada e tem nulos, este ALTER pode falhar.
        -- Considere atualizar nulos para um valor padrão antes, se necessário.
        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = 'chat_messages' AND column_name = 'sender' AND is_nullable = 'YES'
        ) THEN
             -- Primeiro, atualize quaisquer valores NULL existentes para um padrão (ex: 'user') se a lógica permitir.
             -- Exemplo: UPDATE "public"."chat_messages" SET "sender" = 'user' WHERE "sender" IS NULL;
             -- Então, defina como NOT NULL:
             -- ALTER TABLE "public"."chat_messages" ALTER COLUMN "sender" SET NOT NULL;
             -- Por simplicidade e para evitar falha em dados existentes, manteremos a coluna como anulável se recém-criada por este ALTER,
             -- ou você pode adicionar um default e torná-la NOT NULL no ALTER TABLE ADD COLUMN.
             -- Para corresponder ao schema original:
             -- ALTER TABLE "public"."chat_messages" ADD COLUMN "sender" public."chat_sender" NOT NULL; (se não existir)
             -- Se já existe mas é anulável e deve ser NOT NULL:
             -- UPDATE "public"."chat_messages" SET "sender" = 'user' WHERE "sender" IS NULL; -- (escolha um default apropriado)
             -- ALTER TABLE "public"."chat_messages" ALTER COLUMN "sender" SET NOT NULL;
             NULL; -- Deixando como nullable se adicionado aqui, para evitar falhas em dados existentes. O CREATE TABLE original define como NOT NULL.
        END IF;
    END IF;
END
$$;
...
-- Criação de Foreign Keys de forma idempotente
DO $$ BEGIN
