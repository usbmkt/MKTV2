...
CREATE TABLE IF NOT EXISTS "public"."campaigns" (
	...
	"name" text NOT NULL,
	"description" text,
	"status" public."campaign_status" DEFAULT 'draft' NOT NULL, -- Definição original da coluna
...
CREATE TABLE IF NOT EXISTS "public"."chat_messages" (
	...
	"sender" public."chat_sender" NOT NULL, -- Definição original da coluna
	"text" text NOT NULL,
	"attachment_url" text,
...
DO $$
BEGIN
    -- Garante que o tipo ENUM campaign_status exista (necessário para o ALTER TABLE)
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'campaign_status' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        CREATE TYPE "public"."campaign_status" AS ENUM('active', 'paused', 'completed', 'draft');
    END IF;

    -- Adiciona a coluna 'status' à tabela 'campaigns' somente se a tabela existir e a coluna não existir
...
DO $$
BEGIN
    -- Garante que o tipo ENUM chat_sender exista (necessário para o ALTER TABLE)
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'chat_sender' AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        CREATE TYPE "public"."chat_sender" AS ENUM('user', 'agent');
    END IF;

    -- Adiciona a coluna 'sender' à tabela 'chat_messages' somente se a tabela existir e a coluna não existir
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'chat_messages' AND table_schema = 'public') THEN
        IF NOT EXISTS (
            ...
            ALTER TABLE "public"."chat_messages" ADD COLUMN "sender" public."chat_sender" NOT NULL;
        END IF;
    END IF;
END
$$;
