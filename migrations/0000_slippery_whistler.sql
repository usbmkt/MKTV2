...
--> statement-breakpoint
-- Os comandos ALTER TABLE para adicionar FOREIGN KEY não suportam IF NOT EXISTS diretamente.
-- Eles falharão se a constraint já existir, o que é geralmente o comportamento esperado
-- para ferramentas de migração, pois indica que o schema pode estar em um estado inesperado.
-- Se você precisar de idempotência aqui, seria necessário verificar a existência da constraint
-- usando query no information_schema e condicionalmente executar o ALTER TABLE,
-- o que é mais complexo e fora do escopo de uma modificação simples de IF NOT EXISTS.
