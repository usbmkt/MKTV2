...
interface TemplateButton {
  ...
  phoneNumber?: string;
  example?: string[];
  couponCode?: string; // Adicionado para botão de copiar código
}
...
export default function ZapTemplates() {
  ...
  // TODO: Substituir pelos types corretos do seu schema Zod ou Prisma
  const [newTemplateData, setNewTemplateData] = useState<Partial<MessageTemplate>>({
    ...
    components: [
      ...
      { type: 'BODY', text: ''},
      // CORREÇÃO: Adicionando propriedade 'buttons' faltante
  ...
  const createMutation = useMutation<MessageTemplate, Error, Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'>>({
    ...
    onSuccess: () => {
      ...
      setNewTemplateData({ name: '', category: 'UTILITY', language: 'pt_BR', components: [{ type: 'BODY', text: '' }] });
    },
    onError: (err: Error) => {
  ...
  const handleComponentChange = (index: number, field: keyof TemplateComponent, value: any) => {
    setNewTemplateData(prev => {
      ...
      // CORREÇÃO: Aqui estava o erro TS1005, estava `components[index][field] value;`
      // Deveria ser `components[index][field] = value;`
  ...
  const handleButtonChange = (compIndex: number, btnIndex: number, field: keyof TemplateButton, value: string) => {
    setNewTemplateData(prev => {
      const components = [...(prev.components || [])];
      const buttons = [...(components[compIndex]?.buttons || [])];
      // CORREÇÃO: Similar ao handleComponentChange, erro TS1005 aqui também
      (buttons[btnIndex] as any)[field] = value;
      (components[compIndex] as any).buttons = buttons;
      return { ...prev, components };
    });
  };
  ...
  const addTemplateButton = (compIndex: number) => {
    setNewTemplateData(prev => {
      const components = [...(prev.components || [])];
      const buttons = [...(components[compIndex]?.buttons || [])];
      buttons.push({ type: 'QUICK_REPLY', text: 'Nova Resposta' });
      (components[compIndex] as any).buttons = buttons;
      return { ...prev, components };
    });
  };
  ...
  const removeTemplateButton = (compIndex: number, btnIndex: number) => {
    setNewTemplateData(prev => {
      const components = [...(prev.components || [])];
      const buttons = (components[compIndex]?.buttons || []).filter((_, i) => i !== btnIndex);
      (components[compIndex] as any).buttons = buttons;
      return { ...prev, components };
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => { setEditingTemplate(null); setIsModalOpen(true); }} className="neu-button">
