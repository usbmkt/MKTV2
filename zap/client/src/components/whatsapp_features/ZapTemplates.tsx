...
import {
  ...
  DialogFooter,
  DialogDescription,
  // DialogTrigger, // Não usado diretamente aqui, o controle é via isModalOpen
...
export default function ZapTemplates() {
  ...
  const [newTemplateData, setNewTemplateData] = useState<Partial<MessageTemplate>>(JSON.parse(JSON.stringify(defaultNewTemplateDataState)));
  ...
  const handleSelectChange = (name: 'category' | 'language', value: string) => {
    setNewTemplateData(prev => ({ ...prev, [name]: value as MessageTemplate['category'] | MessageTemplate['language'] }));
  };
  
  const handleHeaderFormatChange = (compIndex: number, value: string) => {
    handleComponentChange(compIndex, 'format', value as TemplateComponent['format']);
  };

  const handleButtonTypeChange = (compIndex: number, btnIndex: number, value: string) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const dataToSubmit: Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'> = {
        name: newTemplateData.name,
        category: newTemplateData.category,
        language: newTemplateData.language,
        components: newTemplateData.components || []
    };
  ...
  const getQualityScoreBadgeClass = (scoreData?: MessageTemplate['qualityScore'] | null) => {
    if (!scoreData || !scoreData.score) return 'bg-gray-100 text-gray-700 border-gray-300';
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isModalOpen} onOpenChange={handleModalOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <form id="template-form-id" onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-6 space-y-4 custom-scrollbar">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5">
                    <Label htmlFor="template-modal-name">Nome do Template*</Label>
                    <Input id="template-modal-name" name="name" value={newTemplateData.name || ''} onChange={handleInputChange} placeholder="Ex: promocao_natal_2024" className="neu-input" required />
                ...
                <div className="space-y-1.5">
                    ...
                    <Select name="category" value={newTemplateData.category || 'UTILITY'} onValueChange={(v) => handleSelectChange('category', v as MessageTemplate['category'])}>
                ...
                <div className="space-y-1.5">
                    ...
                    <Select name="language" value={newTemplateData.language || 'pt_BR'} onValueChange={(v) => handleSelectChange('language', v)}>
                    ...
                    {(newTemplateData.components || []).map((comp, compIndex) => {
                        ...
                        return (
                            <div key={compIndex} className="p-3 border rounded bg-card space-y-2">
                                ...
                                {comp.type === 'HEADER' && (
                                    <Select value={comp.format || 'TEXT'} onValueChange={(v: string) => handleHeaderFormatChange(compIndex, v as TemplateComponent['format'])}>
                                        ...
                                        {comp.buttons.map((btn, btnIndex) => (
                                            <div key={btnIndex} className="p-2 border rounded bg-background space-y-1">
                                                <div className="flex justify-between items-center">
                                                    <Select
                                                        value={btn.type}
                                                        onValueChange={(value: string) => handleButtonTypeChange(compIndex, btnIndex, value as TemplateButton['type'])}
                                                    >
          ...
          <DialogFooter className="p-6 pt-4 border-t">
            ...
            <Button
              type="submit"
              form="template-form-id" // Atribuindo o ID do formulário ao botão
