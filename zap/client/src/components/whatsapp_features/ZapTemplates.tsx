...
export default function ZapTemplates() {
  ...
  const createMutation = useMutation<MessageTemplate, Error, Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'>>({
    ...
    onSuccess: () => {
      ...
      setIsTemplateModalOpen(false); // Corrigido
    },
    onError: (err: Error) => {
  ...
  const handleModalOpenChange = (isOpen: boolean) => {
    setIsTemplateModalOpen(isOpen); // Corrigido
    if (!isOpen) {
  ...
  useEffect(() => {
    if (isTemplateModalOpen) { // Corrigido
      if (editingTemplate) {
  ...
  }, [editingTemplate, isTemplateModalOpen]); // Corrigido
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isTemplateModalOpen} onOpenChange={handleModalOpenChange}> {/* Corrigido */}
