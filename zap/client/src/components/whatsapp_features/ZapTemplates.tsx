// client/src/components/whatsapp_features/ZapTemplates.tsx
...
import { useToast } from '@/components/ui/use-toast'; // Ajustei o caminho, presumi que o hook useToast está na pasta ui
...
interface TemplateComponent {
  ...
  example?: {
    header_text?: string[];
    header_handle?: string[]; // Para media URLs
    body_text?: string[][]; // Para variáveis no corpo
  };
...
interface TemplateButton {
  ...
  url?: string;
  phoneNumber?: string;
  example?: string[]; // Para URL buttons
  couponCode?: string; // Para COPY_CODE buttons
}

// Mock data - Em produção, isso viria da sua API
const initialTemplatesFromMock: MessageTemplate[] = [
  {
    id: 'welcome_message_123',
    name: 'welcome_message_123', // Nome deve ser único e seguir padrão do WhatsApp
    category: 'UTILITY',
    language: 'pt_BR',
    status: 'APPROVED',
    components: [
...
const fetchTemplates = async (): Promise<MessageTemplate[]> => {
  ...
  // Retorna uma cópia profunda para evitar mutações diretas no mock
...
const createTemplate = async (templateData: Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'>): Promise<MessageTemplate> => {
  ...
  const newTemplate: MessageTemplate = {
    ...templateData,
    id: `template_${Date.now()}`, // ID único
    status: 'PENDING', // Novo template começa como pendente
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
  initialTemplatesFromMock.push(newTemplate); // Adiciona ao mock
  return newTemplate;
};
...
const deleteTemplateApi = async (templateId: string): Promise<void> => {
  ...
  if (index > -1) {
    initialTemplatesFromMock.splice(index, 1); // Remove do mock
  } else {
...
const defaultNewTemplateDataState: Partial<MessageTemplate> = {
  ...
  language: 'pt_BR',
  components: [
    { type: 'BODY', text: '' }, // Corpo é obrigatório
    // Opcionalmente, comece com botões ou outros componentes se desejar
...
export default function ZapTemplates() {
  ...
  const { data: templates = [], isLoading, error } = useQuery<MessageTemplate[]>({
    ...
    queryFn: fetchTemplates, // Usando a função simulada
  });
  ...
  const createMutation = useMutation<MessageTemplate, Error, Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'>>({
    ...
    onSuccess: () => {
      ...
      setIsModalOpen(false); // Fechar modal ao sucesso
    },
    onError: (err: Error) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Para selects que retornam valores específicos do enum
  ...
  const handleComponentChange = (index: number, field: keyof TemplateComponent, value: any) => {
    setNewTemplateData(prev => {
      ...
      // Criar uma cópia do componente para evitar mutação direta
      const targetComponent = { ...components[index] };
      // Type assertion para permitir atribuição dinâmica
  ...
  const addComponent = (type: TemplateComponent['type']) => {
    ...
    switch(type) {
      case 'HEADER':
        ...
        break;
      case 'BODY': // Body geralmente é único, mas para fins de flexibilidade
      ...
      case 'FOOTER':
        ...
        break;
      case 'BUTTONS':
        newComponent = { type: 'BUTTONS', buttons: [{type: 'QUICK_REPLY', text: 'Resposta Rápida'}]}; // Exemplo de botão
        break;
      default:
  ...
  const addTemplateButton = (compIndex: number) => {
    setNewTemplateData(prev => {
      ...
      } else if (components[compIndex]) { // Se o componente existe mas não tem array de botões ainda
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Remover campos que não devem ser enviados na criação, se existirem
    const { id, status, qualityScore, createdAt, updatedAt, ...dataToSubmit } = newTemplateData;

    createMutation.mutate(dataToSubmit as Omit<MessageTemplate, 'id' | 'status' | 'qualityScore' | 'createdAt' | 'updatedAt'>);
  };
  ...
  const handleModalOpenChange = (isOpen: boolean) => {
    ...
    if (!isOpen) {
      setEditingTemplate(null); // Limpa o template em edição
      setNewTemplateData(defaultNewTemplateDataState); // Reseta para o estado inicial
    }
  };

  useEffect(() => {
    if (isModalOpen) { // Só atualiza o formulário se o modal estiver aberto
      if (editingTemplate) {
        setNewTemplateData(editingTemplate);
      } else {
        setNewTemplateData(defaultNewTemplateDataState); // Garante reset para novo template
      }
    } else {
        // Se o modal foi fechado, garantir que o estado seja o default
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button
          onClick={() => {
            setEditingTemplate(null); // Garante que está criando um novo
            setIsModalOpen(true);
          }}
      ...
      <Dialog open={isModalOpen} onOpenChange={handleModalOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col"> {/* Para melhor visualização do modal */}
          ...
          <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-6 space-y-4 custom-scrollbar">
            ...
            <Card className="neu-card-inset">
                <CardHeader className="pb-2 pt-3">
                    ...
                    <CardDescription className="text-xs">Defina o conteúdo da sua mensagem. Use <code>{"{{1}}"}</code>, <code>{"{{2}}"}</code>, etc. para variáveis.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                    {(newTemplateData.components || []).map((comp, compIndex) => {
                        if (!comp) return null; // Adicionado para segurança
                        return (
                            <div key={compIndex} className="p-3 border rounded bg-card space-y-2"> {/* Usar bg-card para contraste */}
                                <div className="flex justify-between items-center">
                                    <Badge variant="secondary">{comp.type}</Badge>
                                    {/* Permitir remover componentes, exceto o BODY que é obrigatório */}
                                    {comp.type !== 'BODY' && (
                                        <Button type="button" variant="ghost" size="sm" className="h-6 p-1 text-red-500" onClick={() => removeComponent(compIndex)}>
                                ...
                                {comp.type === 'HEADER' && (
                                    <Select value={comp.format || 'TEXT'} onValueChange={(v) => handleComponentChange(compIndex, 'format', v)}>
                                        ...
                                        <SelectContent>
                                            ...
                                            {/* LOCATION não é comum para header de template, mas pode ser adicionado se necessário */}
                                        </SelectContent>
                                    </Select>
                                )}
                                ...
                                {((comp.type === 'HEADER' && comp.format === 'TEXT') || comp.type === 'BODY' || comp.type === 'FOOTER') && (
                                    <Textarea
                                        ...
                                        rows={comp.type === 'BODY' ? 4 : 2} // Mais linhas para o corpo
                                        className="text-sm neu-input"
                                    />
                                )}
                                {/* Lógica para HEADER com mídia (imagem, vídeo, documento) */}
                                {comp.type === 'HEADER' && (comp.format === 'IMAGE' || comp.format === 'VIDEO' || comp.format === 'DOCUMENT') && (
                                    <div className="text-xs text-muted-foreground p-2 border border-dashed rounded bg-muted/50">
                                        ...
                                        <Input
                                            ...
                                            value={(comp.example?.header_handle || [])[0] || ''} // WhatsApp usa header_handle para media URLs
                                ...
                                {comp.type === 'BUTTONS' && comp.buttons && (
                                    <div className="space-y-2">
                                        {comp.buttons.map((btn, btnIndex) => (
                                            <div key={btnIndex} className="p-2 border rounded bg-background space-y-1"> {/* Usar bg-background para contraste */}
                                                <div className="flex justify-between items-center">
                                                    <Select
                                                        value={btn.type}
                                                    ...
                                                    >
                                                        ...
                                                        <SelectContent>
                                                            ...
                                                            {/* CATALOG é mais complexo e pode requerer configuração adicional */}
                                                        </SelectContent>
                                                    </Select>
                                                    <Button
                                                        type="button"
                                                ...
                                                {btn.type === 'COPY_CODE' && (
                                                     <Input
                                                        placeholder="CUPOMXYZ" // Exemplo de código
                                        ...
                                        {(comp.buttons?.length || 0) < 3 && ( // Limite de botões do WhatsApp
                                            <Button
                                                type="button"
                                                variant="outline"
                                                size="sm"
                    ...
                    <div className="flex gap-2 mt-2">
                        ...
                        {/* Permitir adicionar botões apenas se não existir um componente BUTTONS */}
                ...
                <Alert variant="default" className="bg-amber-50 border-amber-200 text-amber-700">
                    <Info className="h-4 w-4 !text-amber-600" /> {/* Ajuste na cor do ícone */}
